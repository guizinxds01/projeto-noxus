    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#0a0a0a] rounded-xl border border-white/5 overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody>
            {categories.length === 0 && (
              <tr><td colSpan={3} className="px-6 py-16 text-center text-gray-600 text-sm">Nenhuma categoria cadastrada.</td></tr>
            )}
            {categories.map(c => (
              <tr key={c._id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                ...
                <td className="px-6 py-4">
                  {c.parent_id ? (
                    <span className="text-[10px] font-black uppercase tracking-widest text-[#00ff88] bg-[#00ff88]/10 px-2 py-1 rounded-md">
                      {categories.find(cat => cat._id === c.parent_id)?.name || 'Desconhecida'}
                    </span>
                  ) : (
                    <span className="text-[10px] font-bold uppercase tracking-widest text-white/20">Principal</span>
                  )}
                </td>
    ...
    <div className="bg-[#0a0a0a] rounded-3xl p-10 border border-white/5 max-w-xl">
      <div className="flex justify-between items-center mb-8">
        <h3 className="text-xl font-black uppercase italic tracking-tighter text-white">
          {current._id ? 'Editar Categoria' : 'Nova Categoria'}
        </h3>
      ...
      <div className="space-y-6">
        {/* Nome */}
        <div>
          <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2 block">Nome da Categoria</label>
          <input
            type="text"
            value={current.name}
            onChange={e => setCurrent(p => ({ ...p, name: e.target.value }))}
            className="w-full bg-[#111] border border-white/5 p-4 rounded-2xl outline-none font-bold text-white focus:border-[#00ff88]/50 text-lg"
        ...
        {/* Categoria Pai */}
        <div>
          <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2 block">Categoria Pai (para subcategorias)</label>
          <select
            value={current.parent_id || ''}
            onChange={e => setCurrent(p => ({ ...p, parent_id: e.target.value || null }))}
            className="w-full bg-[#111] border border-white/5 p-4 rounded-2xl outline-none font-bold text-white focus:border-[#00ff88]/50 text-lg appearance-none"
          >
            <option value="">Nenhuma (Esta é uma Categoria Principal)</option>
            {categories
              .filter(c => !c.parent_id && c._id !== current._id) // Só principais podem ser pais, e não pode ser pai de si mesmo
              .map(c => (
        ...
        {/* Foto */}
        <div>
          <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2 block">Foto da Categoria (opcional)</label>
          <div className="flex items-center gap-6">
            ...
            <div className="flex-1 space-y-2">
              <label className={`flex items-center justify-center gap-2 border border-dashed rounded-2xl px-4 py-4 cursor-pointer transition-all ${uploading ? 'border-[#00ff88]/50 opacity-60' : 'border-white/10 hover:border-[#00ff88]/50'}`}>
              ...
              {current.image && (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 text-red-500/60 hover:text-red-500 text-xs font-bold uppercase tracking-widest py-2 transition-colors"
                >
                  <ImageOff size={14} /> Remover foto
                </button>
              )}
            </div>
          </div>
        ...
        <button
          ...
          className="w-full bg-[#00ff88] text-black py-5 rounded-2xl font-black text-xs uppercase tracking-widest hover:opacity-90 disabled:opacity-50 transition-all"
        >
